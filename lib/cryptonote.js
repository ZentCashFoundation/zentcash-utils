...
// Copyright (c) 2018-2023, The Zent Cash Developers
//
...
const Crypto = require('./zentcash-crypto')
...
class CryptoNote {
  ...
  underivePublicKey (derivation, outputIndex, outputKey) {
    ...
    const [err, key] = ZentCashCrypto.underivePublicKey(derivation, outputIndex, outputKey)
...
function cnFastHash (input) {
  ...
  const [err, hash] = ZentCashCrypto.cn_fast_hash(input)
...
function derivePublicKey (derivation, outputIndex, publicKey) {
  ...
  const [err, key] = ZentCashCrypto.derivePublicKey(derivation, outputIndex, publicKey)
...
function deriveSecretKey (derivation, outputIndex, privateKey) {
  ...
  const [err, key] = ZentCashCrypto.deriveSecretKey(derivation, outputIndex, privateKey)
...
function generateDeterministicSubwalletKeys (privateKey, walletIndex) {
  ...
  const [err, keys] = ZentCashCrypto.generateDeterministicSubwalletKeys(privateKey, walletIndex)
...
function generateKeyDerivation (transactionPublicKey, privateViewKey) {
  ...
  const [err, derivation] = ZentCashCrypto.generateKeyDerivation(transactionPublicKey, privateViewKey)
...
function generateKeyImage (publicKey, privateKey) {
  ...
  const [err, keyImage] = ZentCashCrypto.generateKeyImage(publicKey, privateKey)
...
function checkRingSignatures (transactionPrefixHash, keyImage, inputKeys, signatures) {
  ...
  if (userCryptoFunctions.checkRingSignatures) {
    ...
    return ZentCashCrypto.checkRingSignatures(transactionPrefixHash, keyImage, inputKeys, signatures)
  }
}
...
function generateRingSignaturesPrimitive (transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex) {
  ...
  if (userCryptoFunctions.generateRingSignatures) {
    ...
    const [err, sigs] = ZentCashCrypto.generateRingSignatures(transactionPrefixHash, keyImage, inputKeys, privateKey, realIndex)
...
function generateSignature (digestHash, publicKey, privateKey) {
  ...
  if (userCryptoFunctions.generateSignature) {
    ...
    [err, signature] = ZentCashCrypto.generateSignature(digestHash, publicKey, privateKey)
  }
...
function privateKeyToPublicKey (privateKey) {
  ...
  const [err, key] = ZentCashCrypto.secretKeyToPublicKey(privateKey)
...
function scReduce32 (hex) {
  const [err, result] = ZentCashCrypto.scReduce32(hex)
...
function verifySignature (digestHash, publicKey, signature) {
  ...
  return ZentCashCrypto.checkSignature(digestHash, publicKey, signature)
}
